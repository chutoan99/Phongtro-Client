...
import { menuManageResources } from './resources'
...
const HeaderComponent: NextPage = () => {
	...
	const handleMenuManage = (id) => {
		if (id === 1) {
			Router.push(`${menuManageResources[0].path}`)
		} else if (id === 2) {
			Router.push(`${menuManageResources[1].path}`)
		} else if (id === 3) {
			Router.push(`${menuManageResources[2].path}`)
		} else if (id === 4) {
	...
	return (
		<div className='container container-header clearfix'>
			<Link id='top-logo' href='/' title='cho thuê phòng trọ, cho thuê nhà trọ, tìm phòng trọ'>
			...
			{!isLoading && (
				<div className='user-welcome clearfix js-reload-html-header'>
						...
						<Link className='welcome-text' href='#'>
							...
							<div>
								...
								<span style={{ fontSize: '0.9rem' }}>
									Mã tài khoản: <strong> {dataUser?.id?.slice(0, 15) + `...`}</strong>. TK Chính:
									<strong>0 VNĐ</strong>
								</span>
							</div>
						</Link>
						...
						<div className='dropdown'>
							...
							{isDropDown && (
								<ul className='dropdown-menu'>
									{menuManageResources.map((item: any, index: number) => (
										<li key={index}>
											<Link
					...
					{isLogin && (
						<Link className='btn btn-add-post' href='/admin' style={{ height: '40px' }}>
							Đăng tin mới <i></i>
						</Link>
					)}
				</div>
