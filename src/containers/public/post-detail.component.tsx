...
import { useQueryPostId } from '../../modules/post/hooks'
			...
			<article className='the-post tin-vip vipnoibat'>
					...
					<div className='images-swiper-container swiper-container-horizontal'>
						<Slider {...settings}>
							{data?.listImage?.image &&
								JSON.parse(data?.listImage?.image)?.map((item: any, index: number) => (
				...
				<header className='page-header'>
					...
					<p className='margin-bottom: 10px;'>
						Chuyên mục:
						<Link style={{ textDecoration: 'underline' }} title='Phòng trọ Quận Tân Phú' href=''>
					...
					<div className='post-attributes'>
						...
						<div className='item published'>
							<i></i>
							<span title='Thứ 5, 08:31 23/02/2023'>{data?.attributes?.published}</span>
						</div>
				...
				<section className='section post-main-content'>
					...
					<div className='section-content'>
						{data?.description &&
							JSON.parse(data?.description)?.map((ele: any, index: number) => <p key={index}>{ele}</p>)}
					</div>
				</section>
						...
						<table className='table'>
							<tbody>
								...
								<tr>
									...
									<td>
										<span style={{ color: '#E13427' }}>{data?.overviews?.bonus}</span>
									</td>
								</tr>
								<tr>
									<td className='name'>Ngày đăng:</td>
									<td>
										<time title='Thứ 5, 08:31 23/02/2023'>{data?.overviews?.created}</time>
									</td>
								</tr>
								<tr>
									<td className='name'>Ngày hết hạn:</td>
									<td>
										<time title='Thứ 6, 08:31 10/03/2023'>{data?.overviews?.expired}</time>
									</td>
								</tr>
							</tbody>
						</table>
				...
				<section className='section post-map'>
					<div className='section-header'>
						...
						<address className='section-description'>Địa chỉ: {data?.address}</address>
					</div>
					<div className='section-content'>
						<div
							id='__maps_content'
							...
							<iframe
								...
								height='100%'
								style={{ border: 0 }}
								src='https://www.google.com/maps/embed/v1/place?q=24 Đường Sơn Kỳ, Phường Sơn Kỳ, Quận Tân Phú, Hồ Chí Minh&amp;key=AIzaSyD6Coia3ssHYuRKJ2nDysWBdSlVlBCzKAw&amp;zoom=14'></iframe>
						</div>
					</div>
				</section>
				...
				<section className='section post-report'>
					<div className='section-content'>
						<p>
							...
							<em>
								"CHO THUÊ PHÒNG TRỌ MỚI CHÍNH CHỦ, GIẢM GIÁ, QUẬN TÂN PHÚ - GẦN BÊN TRƯỜNG ĐẠI HỌC CÔNG
								NGHỆ THỰC PHẨM - Mã tin: 135699"
							</em>
							. Mọi thông tin liên quan đến tin đăng này chỉ mang tính chất tham khảo. Nếu bạn có phản hồi
							với tin đăng này (báo xấu, tin đã cho thuê, không liên lạc được,...), vui lòng thông báo để
							PhòngTrọ123 có thể xử lý.
						</p>
						<a className='btn btn-report btn-outline' target='_blank' rel='nofollow' href=''>
							<i></i> Gửi phản hồi
						</a>
					</div>
				</section>
				...
				<div className='post-fix-bar'>
					<div className='inner clearfix'>
						...
						<div className='right-bar'>
							<span className='post-fix-bar-save js-btn-save' data-post-id='135699'>
								<i></i> Yêu thích
							</span>
							<a
