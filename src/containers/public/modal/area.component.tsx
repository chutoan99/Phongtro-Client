...
import { convert100ToTarget, convertTo100, getNumbersArea } from '../../../utils/Commom/fomarNumber'

function ModalAreaComponent({ items, modals, setOverPlay, setModals, handleSubmit, arrMinMax }) {
	let number = +90
	const [perSent1, setPerSent1] = useState(arrMinMax?.areaArr?.length > 0 ? arrMinMax?.areaArr[0] : 0)
	const [perSent2, setPerSent2] = useState(arrMinMax?.areaArr?.length > 0 ? arrMinMax?.areaArr[1] : 100)
	...
	const handleBeforeSubmit = () => {
		let min = perSent1 <= perSent2 ? convert100ToTarget(perSent1, 'area') : convert100ToTarget(perSent2, 'area')
		let max = perSent1 <= perSent2 ? convert100ToTarget(perSent2, 'area') : convert100ToTarget(perSent1, 'area')
		handleSubmit(
			{
				[`${'area'}Number`]: [min, max],
				['area']: `Từ ${min} - ${max} ${'m2'}`
...
export default memo(ModalAreaComponent)
