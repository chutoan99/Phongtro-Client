...
function ModalCategoryComponent({ items, isModals, setModals, setOverPlay, handleSubmit, queries }) {
						...
						<div id='filter-popup-estate-type-option' className='filter-list-option'>
							<ul>
								{items?.map((item: any, index: number) => (
									<li key={index} className='selected' onClick={() => onSubmit(item)}>
										<input
											type='radio'
											name='category'
											id={item?.code}
											value={item?.code}
											defaultChecked={item.code === queries[`${'category'}Code`]}
...
export default memo(ModalCategoryComponent)
