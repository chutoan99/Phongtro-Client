...
export default function AdminManagerPostComponent() {
	...
	const checkStatus = (dateTime) => moment(dateTime, 'DD/MM/YYYY').isSameOrAfter(new Date().toDateString())
	return (
		<>
			<div className='d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pb-2 mb-3'>
				<h1 className='h2'>Quản lý tin đăng</h1>
				<div className='btn-toolbar mb-2 mb-md-0 d-flex' style={{ gap: '10px' }}>
				...
				<div className='table-responsive'>
					<table className='table table_post_listing table-bordered _table-hover'>
						<thead>
							<tr>
								<th>Mã tin</th>
								<th style={{ textAlign: 'center', whiteSpace: 'nowrap' }}>Ảnh đại diện</th>
								<th>Tiêu đề</th>
								<th>Giá</th>
								...
								{dataPosts.post.map((item: any, index: number) => (
									<tr key={index}>
										...
										<td>
											<div className='post_thumb'>
												<Link href='#' target='_blank'>
													<img src={item?.listImage?.postImg} alt={item?.title} />
												</Link>
											</div>
										</td>
										<td>
											{item?.categoryCode === 'NCT' && (
												<span className='badge badge-pill badge-primary '>Nhà cho thuê</span>
											)}
											...
											<div className='post_btn_toolbar mt-3 '>
												...
												<button className='btn btn-sm' onClick={() => onDeletePOst(item)}>
													<svg
														xmlns='http://www.w3.org/2000/svg'
														width='24'
														height='24'
										...
										<td
											...
											<div className='post_price'>{item?.attributes?.price}</div>
										</td>
										<td
											style={{
												textAlign: 'center'
										...
										<td
											...
											<span className='text text-warning' style={{ whiteSpace: 'nowrap' }}>
												Chưa thanh toán
											</span>
										</td>
									</tr>
